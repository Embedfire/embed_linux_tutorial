...
参考前面《fire-config工具简介》章节，使能MPU6050模块。MPU6050模块依赖于IIC通信，
当MPU6050模块被使能后，IIC设备驱动也将一起被使能。
...
使能MPU6050模块后，我们可以通过IIC设备文件来检查设备的一些基本信息，下面简单介绍几个查看命令。
...
I.MX6ULL有4个I2C外设，MPU6050连接到了它的IIC 1接口，对应的驱动设备文件为上图中的i2c-0，
这是因为驱动是从0开始编号的。 因此应用程序一般通过操作设备文件i2c-0实现对MPU6050的操作。
不止MPU6050，理论上任何连接到 IIC 1 接口上的设备（触摸、摄像头、oled）都可以通过 i2c-0 设备文件来实现具体的功能。
...
参考前面《fire-config工具简介》章节，使能OLED模块。MPU6050模块依赖于IIC通信，
当MPU6050模块被使能后，IIC设备驱动也将一起被使能。

如下图:

.. image:: media/i2c/i2cbus024.png
   :align: center
   :alt: 未找到图片
