...
在Linux内核文档的Documentation/SPI目录下有关于SPI驱动非常详细的说明本章节的示例代码目录为：
~/base_code/linux_app/spi
...
将会在SCK时钟线的“奇数边沿”被采样。当CPHA=1时，数据线在SCK的“偶数边沿”采样。见下图。
...
类似地，当CPHA=1时，不受CPOL的影响，数据信号在SCK的偶数边沿被采样，见下图。
...
通过键盘的上、下方向键选中 “ecspi3”， ecspi3是本章需要的设备，它使用的是SPI3 。通过
键盘左、右 键选中 “select ”选项，然后按回车，如下所示：


.. image:: media/spiapp007.png
   :align: center
   :alt: 未找到图片


.. image:: media/spiapp008.png
   :align: center
   :alt: 未找到图片
...
SPI_OLED 显示屏的 D/C 引脚用于控制SPI写入的是命令还是数据，所以我们需要另
外一个引脚，将它连接到SPI_OLED的 D/C引脚。为简化程序设计，我们直接使用RGB灯的“绿灯”控
制引脚，这里要添加LED设备（如果已经添加，请忽略该步骤）。

GPIO添加方法与SPI添加方法基本相同，不同的是使用“fire_config”打开配置界面后选择“LED”如下所示。

.. image:: media/spiapp010.png
   :align: center
   :alt: 未找到图片
...
验，我们只需要将SPI 3的 MIOS与MOSI引脚使用跳线帽短接即可。SPI 3 引脚说明以及硬件连接如下所示。
...
(2) 第二部分，调用ioctl执行发送，参数fd,是SPI设备文件描述符，参数SPI_IOC_MESSAGE(1)用于指定执
...
以上程序的源码位于 ~/base_code/linux_app/spi/spi_test 目录下，在该目录
下执行 make ，编译完成后在 “~/ spi_test /bin”目录下生
...
的代码请参考本小节配套源码（位于se_code/section2/spi_test 目录下）。
...
以上程序的源码位于 ~/base_code/linux_app/spi/spi_oled 目录下
，在该目录下执行 make ，编译完成后在 “~/ spi_oled /bin”目录下生
